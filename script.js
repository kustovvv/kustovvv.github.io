...
function showDefaultDayTimes() {
    eventStartEnd.style.display = 'flex';
    minusSymbol.style.display = 'none';
    eventDateEnd.style.display = 'none';
}

function showAllDayTimes() {
    eventStartEnd.style.display = 'none';
    minusSymbol.style.display = 'flex';
...
function formatGoogleCalendarDaysFullBlocked() {
//    function convertTime(newDate, time) {
//        times = time.split('-');
//        let newStartTime = concatenateDateTime(newDate, times[0]);
//        let newEndTime = concatenateDateTime(newDate, times[1]);
...
//    }
    ...
    console.log('googleCalendarEventsFullBlocked: ', googleCalendarEventsFullBlocked)
    googleCalendarEventsFullBlocked.forEach(function(calendarEvent) {
        let start_date = new Date(calendarEvent.start_date);
        let end_date = new Date(calendarEvent.end_date);
        createCalendarDays(start_date, end_date);
    })
}
...
function get_existing_unavailable_time() {
    ...
    if (data.busy_times) {
        ...
        googleCalendarEventsFullBlocked = data.busy_times_all_blocked;
        manipulate();
        formatGoogleCalendarDays();
        formatGoogleCalendarDaysFullBlocked();
        }
    })
...
function processContextMenuPopup(clickedElement) {
    ...
    console.log('y: ', y)
    console.log('x_left: ', x)
    console.log('calendarElement.clientWidth: ', calendarElement.clientWidth)
...
function createRecurrentEvents(existingEvents) {
    ...
    recurrent_events.forEach(function(element) {
        var isExistingEvent = existingEvents.some(function(event) {
            if (event.allDay) {
                if (element.length === 3) {
                    return isExistingAllDayEventFunc(event, element[0])
                } else {
                    return isExistingDefaultEventFunc(event, element[0], element[1]);
                }
            } else {
                return isExistingDefaultEventFunc(event, element[0], element[1]);
            }
